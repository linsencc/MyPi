...
import type { Unit, UnitRefreshMode } from "@/data/demo-data"
import type { UnitScheduleFormState } from "@/lib/apply-unit-schedule"
...
export function EditUnitDialog({
  ...
  onSave: (unitId: string, form: UnitScheduleFormState) => void
}) {
  ...
  useEffect(() => {
    if (!open || !unit) return
    setFormRefreshMode(unit.refreshMode)
    const { value, unit: u } = intervalSecondsToForm(unit.intervalSeconds)
    setFormIntervalValue(value)
    setFormIntervalUnit(u)
    setFormScheduledClock((unit.scheduledClock ?? "").trim() || parseClockFromNextRefresh(unit.nextRefresh))
    setFormWeekdays(normalizeWeekdaysSelection(unit.scheduledWeekdays))
  }, [open, unit])

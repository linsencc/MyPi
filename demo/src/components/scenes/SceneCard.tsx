...
/** 与 `scene` 引用解耦：仅 src/alt 变才重绘预览层，减轻连点「立即渲染」时的整区闪动 */
...
export const SceneCard = memo(function SceneCard({
  ...
  return (
    ...
    >
      <SceneCardThumbnail src={previewSrc(scene, "thumb")} alt={`${scene.name} 缩略预览`} />
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                disabled={renderBusy}
                aria-label={`立即渲染并优先上墙「${scene.name}」`}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={`编辑间隔或定时刷新「${scene.name}」`}
                onMouseDown={(e) => {
                  if (e.button === 0 && e.detail > 1) e.preventDefault()
                }}

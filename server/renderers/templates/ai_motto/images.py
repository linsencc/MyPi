"""Remote art for 每日寄语: pinscrape (optional) → Pinterest v5 API → optional stock / offline.

Pinscrape: ``MYPI_MOTTO_PINSCRAPE`` / ``MYPI_MOTTO_PINSCRAPE_AUTO``, ``MYPI_PINSCRAPE_*``,
``MYPI_LLM_PROXY`` / ``HTTPS_PROXY`` (see ``net.http_proxy_url``). API: ``MYPI_PINTEREST_*``.
Fallbacks: ``MYPI_MOTTO_STOCK_FALLBACK``, ``MYPI_MOTTO_OFFLINE_IMAGE``. Images honor
``MYPI_MOTTO_IMAGE_NO_PROXY`` via ``build_motto_image_opener``.
"""
...
def _pinscrape_should_try() -> bool:
    """Pinscrape: explicit on/off, else auto when HTTP proxy is set and pinscrape is installed."""
    raw = os.environ.get("MYPI_MOTTO_PINSCRAPE", "").strip().lower()
    if raw in ("0", "false", "no", "off"):
        return False
    if raw in ("1", "true", "yes", "on"):
        return True
    auto = os.environ.get("MYPI_MOTTO_PINSCRAPE_AUTO", "1").strip().lower()
    if auto in ("0", "false", "no", "off"):
        return False
    return bool(http_proxy_url()) and _pinscrape_import_ok()
...
def _pinscrape_search_query_tags(image_prompt: str) -> str:
    """Short tag-led query tuned for in.pinterest.com web search (not partner API blob)."""
...
def fetch_pinterest_motto_image(image_prompt: str, motto: str) -> Image.Image | None:
    if _pinscrape_should_try():
        ps = _fetch_via_pinscrape(image_prompt, motto)
        if ps is not None:
            return ps
        log.info("ai_motto: pinscrape yielded no image; falling back to Pinterest API if token is set")

    token = pinterest_access_token()
    if not token:
        log.warning("ai_motto: MYPI_PINTEREST_ACCESS_TOKEN not set; cannot use Pinterest API")
        return None

...
MOTTO_STRATA: tuple[MottoStratum, ...] = (
    ...
    MottoStratum(
        "film_jp_kr",
        "【本次唯一维度·影视·日韩】仅选：日本或韩国**真人影视**台词的**中文通行译本**（书面汉语）；**出处须为《片名》。** **禁止**在 motto 中出现日文假名、韩文谚文或原文非中文对白；**禁止**华语、好莱坞、动画长片（本片种为真人）。"
        + _MOTTO_FORMAT_HINT,
    ),
    MottoStratum(
        "film_europe_row",
...
RETRY_DIVERSIFY_SUFFIX = (
    ...
    f"motto 仍须：**简体中文**整句（**无**日文假名、韩文谚文），`「正文」 -- 出处`（直角引号、半角空格、ASCII 双减号 --）；**整行 ≤{MOTTO_MAX_CHARS} 字符**。"
)

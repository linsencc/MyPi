...
SYSTEM_PROMPT = textwrap.dedent(f"""\
    ...
    1. **语言**：`motto` **整句主体须为简体中文**（汉字为主，含常用中文标点）；`「…」` 内为中文摘句或通顺的**中文译文**；`--` 后出处用中文习惯写法（外国作者写通行中文译名即可）。**禁止**以英文、日文假名块或其它非中文自然语言作为正文主体；禁止整句拉丁字母口号。
...
MOTTO_CHINESE_ENFORCEMENT = textwrap.dedent("""\
    【中文纠偏】上一稿 **不是合格的中文寄语**：须输出**简体中文**整句，`「…」` 内为中文或通顺中译；`--` 后为中文出处习惯；禁止英文/其它语言作主句。仍须满足系统提示中的版式与字数。
""")

USER_PROMPT = textwrap.dedent(f"""\
    请按**用户消息中本轮的【本次唯一维度】与【近期去重】**生成一条新的**简体中文**寄语（不要输出英文句子作正文）。

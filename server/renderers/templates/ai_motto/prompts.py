...
SYSTEM_PROMPT = textwrap.dedent(f"""\
    ...
    2. 整行唯一格式：`「正文」 -- 出处`。JSON 字符串内**不得**含换行；「」内为**一段完整可读的正文**（可用逗号、分号、句号衔接**两三句**，或一条稍长的经典摘句），**忌过短**（避免仅十余字像标题口号）
    ...
    6. 长度：从首字「到出处末字，**总长 ≤ {MOTTO_MAX_CHARS} 个字符**（计空格与标点）；在上限内**尽量写满**：优先信息量足、可单独成段的摘句，而非一句极简带过
    7. 内容：真实作品或公认可核对的摘句；**正文宜充实、成段**，仍洗练、忌口号堆砌；汉译外文读起来要像中文书面语
...
USER_PROMPT = textwrap.dedent(f"""\
    ...
    - motto：严格使用 `「正文」 -- 出处` 版式；正文宜**充实成段**（非一句即止），**整行 ≤ {MOTTO_MAX_CHARS} 字符**；真实摘句、忌空泛鸡汤。
""")

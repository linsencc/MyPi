...
def _system_prompt(max_lines: int) -> str:
    return (
        "你是深圳本地活动信息助理。请根据你的最新检索能力，列出**自今天起 31 天内**在深圳市（含南山、福田等）"
        "可参与的**真实**公众活动：展览、音乐会、戏剧、市集、讲座等。"
        "不要列出已超过今天或明显晚于一个月的活动；不得编造具体场馆与日期；不确定则少列。"
        "只输出一个 JSON 对象，键 `lines` 为字符串数组。"
        "每条**必须且仅能**用中文竖线「｜」分成四段，顺序固定："
        "「M/D 周X｜类型｜活动标题｜地址」。"
        "其中第一段为公历月/日加空格加周几（示例：4/25 周六）；类型为两字左右（展览、市集、讲座、戏剧等）；"
        "地址为区+道路/门牌级可核对表述，若无法核实则写「待核实」勿留空。"
        "每条总长不超过 120 个汉字。"
        "``lines`` 必须按活动开始日期**由近到远**排序（越临近今天越靠前；说不准具体日期的条目放在末尾）。"
...
def fetch_shenzhen_event_lines(*, max_lines: int) -> list[str]:
    ...
    user = (
        f"请列出深圳**自今天起 31 天内**最多 {max_lines} 条活动，严格只输出 JSON；"
        "只含未过期的场次；按开始日期**由近到远**排列（最近的最前）。"
        "每条四段用「｜」分隔，含日期周几、类型、标题、地址。例如："
        '{"lines":["4/25 周六｜展览｜深圳博物馆常设展｜福田区福中路市民中心A区"]}'
    )
    payload = json.dumps(
        {
            "model": model,

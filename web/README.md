...
对接本仓库 [pi-server](../pi-server) 的 `GET/PUT /api/v1/*`，无本地 mock 场景数据。界面与交互参考 `demo/`，但 **`demo/` 不参与构建**，请勿从本包引用 `demo` 路径。

## 开发

1. 启动后端（仓库根目录下 `pi-server`，默认 `5050`）：
...
产物在 `web/dist/`。可由 nginx 反代静态资源，或由 Flask 托管该目录（需在 pi-server 侧自行配置）。
...
- 若要在浏览器中展示真实渲染缩略图，需要在 **pi-server** 增加静态文件或签名 URL 路由，并在编排器写入可访问的 `currentPreviewUrl`（或等价字段）。此为后端增强，与本文档并列排期即可。
